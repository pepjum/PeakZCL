...
OPTIONS:
   ...
   -r   Filter applied at counts level (VALUES: mean or median. Default="median")
EOF
}
...
maxxxpeak="1000000"
filter="median"
while getopts "f:c:n:w:l:s:g:t:z:j:b:a:k:x:o:v:y:m:p:r:" OPTION
do
	case $OPTION in
	f) sdir=$OPTARG
	;;
...
##launch Rscript for each chromosome
 ...
 if [[ ! -z $cdir ]]; then
   while IFS='' read -r line || [[ -n "$line" ]]; do        # go through the file to get every line
                ...
                outnamefileR=$line
                printf "Running Rscript..... $sPath/PeakZCL2.R -f $sdir/TMP/chip/$line.chrom -c $sdir/TMP/input/$line.chrom -n "$normalization" -j "$clustering" -w "$decimating" -l "$length" -s "$scales" -g "$levth" -z "$enrichment" -a "$area" -k "$minxxpeak" -x "$maxxxpeak" -m "$allpeaks" -o "$outnamefileR"  \n" &
                Rscript $sPath/PeakZCL2.R -f $sdir/TMP/chip/$line.chrom -c $sdir/TMP/input/$line.chrom -n "$normalization" -w "$decimating" -l "$length" -s "$scales" -g "$levth" -z "$enrichment" -j "$clustering" -a "$area" -k "$minxxpeak" -x "$maxxxpeak" -m "$allpeaks" -o "$outnamefileR"  &
                NPROC=$(($NPROC+1))
     ...
     while IFS='' read -r line || [[ -n "$line" ]]; do
               outnamefileR=$line
               printf "Running Rscript ..... $sPath/PeakZCL2.R -f $sdir/TMP/chip/$line.chrom -n "$normalization" -j "$clustering" -w "$decimating" -l "$length" -s "$scales" -g "$levth" -z "$enrichment" -a "$area" -k "$minxxpeak" -x "$maxxxpeak" -m "$allpeaks" -o "$outnamefileR"  \n" &
               Rscript $sPath/PeakZCL2.R -f $sdir/TMP/chip/$line.chrom -n "$normalization" -w "$decimating" -j "$clustering" -l "$length" -s "$scales" -g "$levth" -z "$enrichment" -a "$area" -k "$minxxpeak" -x "$maxxxpeak" -m "$allpeaks" -o "$outnamefileR"   &
               NPROC=$(($NPROC+1))
               if [ "$NPROC" -ge "$cores" ]; then
                 wait
                 NPROC=0
...
Rscript $sPath/model.R $outdir$outnamefile.Norm $filter
wait
...
if [[ ! -z $cdir ]]; then
    ...
    control=TRUE
    Rscript $sPath/Poisson2.R $outdir$outnamefile.model $allpeaks $klustering $control $maxpeak $minpeak
